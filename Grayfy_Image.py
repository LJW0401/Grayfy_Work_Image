...
#初始化函数
    ...
    #操作按键区域
        ...
        #继续降噪
        self.Button_Noise_Reduction=tk.Button(
            Frame_Button_Handling,
            text='图像降噪',
            font=('黑体',15),
            command=self.Button_Noise_Reduction_Func
        )
        self.Button_Noise_Reduction.pack(side='left',padx=50)
...
#控件函数
    ...
    def Button_Noise_Reduction_Func(self):
        self.GrayImg = cv2.medianBlur(self.GrayImg, 3)
        self.GrayImg = cv2.bilateralFilter(self.GrayImg, 7, 75, 75)
        self.Show_Img()
